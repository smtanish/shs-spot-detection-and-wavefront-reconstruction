# This file provides a real-time 3D visualization of the reconstructed wavefront.
# It receives wavefront data from the infer_unet ie inference pipeline, computes display metrics, and renders the surface using OpenGL.
# The visualization is designed for live use, with a fixed mesh that is reused across frames so that only vertex heights and colors are updated during rendering
# Decoupling computation and rendering ensures smooth display
...
class LiveWavefrontViewer(QtCore.QObject):
    def __init__(self, grid_size=150, fps=30):
        super().__init__()
        # Main application window that holds everything
        self.container = QtWidgets.QWidget()
        self.container.setWindowTitle("Live Wavefront")
        self.container.resize(900, 700)
        # OpenGL view used for hardware-accelerated 3D rendering
        # Significantly faster than using Matplotlib for real-time updates
        ...
        # HUD label displaying physical wavefront metrics (PV and RMS)
        ...
        self.hud.adjustSize()
        self.hud.move(16, 16)
        # Prevent the HUD from blocking mouse interaction with the 3D view
        self.hud.setAttribute(QtCore.Qt.WidgetAttribute.WA_TransparentForMouseEvents)
        self.hud.raise_()
        self.hud.show()
        # HUD label displaying how long each render step takes
        ...
        self.hud_render.move(16, 44)
        self.hud_render.setAttribute(
            QtCore.Qt.WidgetAttribute.WA_TransparentForMouseEvents
        )
        self.hud_render.show()
        self.hud_render.raise_()
        # Create a square grid in normalized pupil coordinates [-1, 1]
        # This grid defines where the wavefront surface will be drawn
        self.x = np.linspace(-1.0, 1.0, grid_size)
        self.y = np.linspace(-1.0, 1.0, grid_size)
        self.nx = self.ny = grid_size
        Xg, Yg = np.meshgrid(self.x, self.y, indexing="xy")
        # Boolean mask defining the circular pupil region
        # Points outside the pupil are excluded from the mesh
        self.pupil = (Xg**2 + Yg**2) <= 1.0
        # Precompute the mesh topology once
        # This avoids rebuilding geometry every frame 
        verts = []
        faces = []
        # Map from grid indices (i, j) to OpenGL vertex indices
        self._vertex_map = -np.ones((self.ny, self.nx), dtype=int)
        v = 0
        # Create one vertex for each grid point inside the pupil
        for j in range(self.ny):
            for i in range(self.nx):
                if self.pupil[j, i]:
                    ...
                    v += 1
        # Create triangular faces between neighboring vertices
        # Each square cell inside the pupil becomes two triangles
        ...
        # Store vertex positions, face indices, and per-vertex colors
        ...
        # Create OpenGL mesh data object
        self._meshdata = gl.MeshData(
            vertexes=self._verts,
            faces=self._faces
        )
        self._meshdata.setVertexColors(self._colors)
        # Create the actual drawable OpenGL mesh item
        self.mesh_item = gl.GLMeshItem(
            meshdata=self._meshdata,
            drawEdges=False,
            smooth=False
        )
        # Scale Z axis so height variations are visually reasonable
        self.mesh_item.scale(1.0, 1.0, 0.02)
        self.view.addItem(self.mesh_item)
        # Add coordinate axes for orientation
        self.view.addItem(gl.GLAxisItem())
        # Shared state used to safely pass results from compute thread to render thread
        self._latest_result = None
        self._lock = QtCore.QMutex()
        # Timer that triggers rendering at a fixed frame rate
    ...
    # Called by the inference thread to submit a new result
    def submit(self, res):
        self._lock.lock()
        self._latest_result = res
        self._lock.unlock()
    # Called periodically by Qt to check for new data and render it
    ...
    # Perform the actual rendering using the prebuilt mesh
    def _render(self, W_vis, W_phys, X, Y):
        import time
        t0 = time.perf_counter()
        # Remove piston and tilt before computing physical metrics
        W = remove_piston_tilt(W_phys, X, Y)
        finite = np.isfinite(W)
        if not np.any(finite):
            return
        # Compute physical wavefront metrics in nanometers
        pv = (np.nanmax(W) - np.nanmin(W)) * 1e9
        rms = np.sqrt(np.nanmean(W[finite] ** 2)) * 1e9
        self.hud.setText(f"PV = {pv:.1f} nm | RMS = {rms:.1f} nm")
        self.hud.adjustSize()
        self.hud.raise_()
        # Normalize wavefront for color mapping
        ...
        # Update vertex heights and colors only
        # Geometry and connectivity never change
        for j in range(self.ny):
            for i in range(self.nx):
